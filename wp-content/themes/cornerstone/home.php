      ...
      ?>
       <?php if (get_field('body_on_front',$section->ID)) { ?>
          <?php 
           ...
           ?>
      <?php } else { ?>
        <div id="section-<?=$section_id;?>" class="grid-container full section <?php !empty(get_field('background_image', $section->ID)) ? 'white-text blue-bg' : '' ?>" style='background-image: url("<?=get_field('background_image', $section->ID); ?>"); background-repeat: no-repeat;
